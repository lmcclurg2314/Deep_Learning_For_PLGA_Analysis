...
markerNames.append("Dox_Ab")
y = metaData.DOX_Ab.values

# Making dataframe from cytoData
list_all = list() # should be a (35000, 18) list where 18th feature is DOX_Ab
for i in range(len(cytoData)): # 35 samples
...
X = dataframe_all.loc[:, dataframe_all.columns != "Dox_Ab"]
y = dataframe_all.loc[:, dataframe_all.columns == "Dox_Ab"]
...
# From the above results, we know to use the following markers (just used all)
cols = ['FSC-A', 'SSC-A', 'SSC-H', 'SSC-W', 'PI-B-630/40-A', 
        'PerPCy55-B-695/40-A','APC-R-670/14-A', 'APC-Cy7-R-780/60-A',]
X = X[cols]
y = y["Dox_Ab"] # y remains the same
